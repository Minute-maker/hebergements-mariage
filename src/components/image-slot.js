/**
 * <image-slot> — emplacement d'image que l'utilisateur remplit lui-même.
 *
 * Repris du composant fourni avec le design (design-handoff/project/image-slot.js).
 * Seule la persistance change : le prototype écrivait un fichier « sidecar » via le
 * pont de l'outil de design, cette version enregistre dans le stockage local du
 * navigateur (clé « mariage.image-slots »). Le reste — dépôt d'image, recadrage au
 * double-clic, glisser pour déplacer, molette ou poignées pour zoomer, Échap pour
 * valider — est inchangé.
 */
/* BEGIN USAGE */
/**
 * <image-slot> — user-fillable image placeholder.
 *
 * Drop this into a page wherever a design needs an image the user supplies.
 * You control the slot's shape; it sizes to its container by default. The user
 * fills it by dragging an image file onto it (or clicking to browse), and the
 * dropped image persists across reloads in the browser's local storage, under a
 * single shared key. Every slot needs a distinct id, which is its entry in that
 * store — the images stay on the device and are never uploaded anywhere.
 *
 * Attributes:
...
(() => {
  const STATE_FILE = 'mariage.image-slots';
  ...
  // ── Shared store ────────────────────────────────────────────────────────
  // One entry per <image-slot> on the page, all under a single local-storage
  // key, written immediately on change.
  const subs = new Set();
  let slots = {};
  // ids explicitly cleared before the store was read — otherwise the merge
  // below can't tell "never set" from "just deleted" and would resurrect the
  // stale stored value.
  ...
  function load() {
    ...
    if (stored && typeof stored === 'object') {
      // Le stockage perd contre toute modification déjà faite en mémoire
      // (dépôt ou effacement), pour qu'aucun des deux n'écrase l'autre.
      const merged = Object.assign({}, stored, slots);
      for (const k in slots) {
        if (merged[k] && !merged[k].u && stored[k]) {
          merged[k].u = typeof stored[k] === 'string' ? stored[k] : stored[k].u;
        }
      }
  ...
  function write() {
    try {
      localStorage.setItem(STATE_FILE, JSON.stringify(slots));
    } catch (e) {
      // Quota dépassé : l'image reste affichée pour la session, sans être conservée.
      console.warn('<image-slot> : image non conservée (stockage local plein).');
    }
  }
  ...
  class ImageSlot extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // Re-render on hover so the controls reliably appear.
